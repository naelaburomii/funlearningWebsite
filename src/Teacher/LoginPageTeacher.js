...
function LoginPageTeacher() {
  ...
  const handleIDChange = (event) => {
    ...
    if (/^\d{0,9}$/.test(value)) {
      setID(value);
      setErrors({ ...errors, ID: value.length === 9 ? '' : 'תעודת זהות חייבת להיות בדיוק 9 ספרות' }); // real-time validations 
    } else {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/LoginPageTeacher', {
        ID,
        password,
        userType: 'Teacher'
      });
      ...
      if (response.status === 200) {
        ...
        setTimeout(() => {
          navigate('/DashboardTeacher'); // הממשתמש יעבור לדף בית של מורה!
